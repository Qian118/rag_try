...
while True:
    ...
    print(prompt)
    response, _ = chat_model.chat(chat_tokenizer, prompt+"根据上述治疗方案，给出下述病情的治疗方案，回答仅输出生成治疗方案的内容，不得输出无关内容。"+query,history=[])#回答不得出现给出案例治疗方案的内容。
    response1, _ = chat_model.chat(chat_tokenizer, "给出下述病情的治疗方案，回答仅输出生成治疗方案的内容，不得输出无关内容。"+query,history=[])
    print ('--->挂载前',response1)
    print ('--->挂载后',response)
# similar_querys=get_similar_query(query)
